...
namespace BootstrapSupport
{
    public static class DefaultScaffoldingExtensions
    {
        ...
        public static string IdentifierPropertyName(this Type type)
        {
            ...
            if (properties.Any(x => Attribute.IsDefined(x, typeof(System.ComponentModel.DataAnnotations.KeyAttribute), false)))
            {
                return
                    properties.Select(x => x)
                    .Where(x => Attribute.IsDefined(x, typeof(System.ComponentModel.DataAnnotations.KeyAttribute), false)).First().Name;
            }
            else if (properties.Any(p => p.Name.Equals("id", StringComparison.CurrentCultureIgnoreCase)))
            {
                return
                    properties.First(p => p.Name.Equals("id", StringComparison.CurrentCultureIgnoreCase)).Name;
            }
            return "";
        }
